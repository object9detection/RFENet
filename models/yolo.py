...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        #     nn.Sequential(
        #         MCSPMLP(x, self.no * self.na),
        #         nn.Conv2d(self.no * self.na, self.no * self.na, 1)
        #     ) for x in ch
        # ])
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            self.model.to(torch.device('cuda'))   # 新加
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            # FEM放在检测头
            # mi[-1].bias.data[:] = 1.0
            # mi[-1].bias.data[:m.nc] = math.log(5 / m.nc / (640 / s) ** 2)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, CSPMLP, MCSPMLP, MA2, MCSPMLP2, CSPMLP2, ACSP, CSPConv, DCNV3_YoLo, C3_DCNV3]:
