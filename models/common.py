...
class DepthWiseConv_3(nn.Module):
    def __init__(self, in_channel, out_channel):
        # 这一行千万不要忘记
        super(DepthWiseConv_3, self).__init__()
...
class DepthWiseConv_5(nn.Module):
    def __init__(self, in_channel, out_channel):
        # 这一行千万不要忘记
        super(DepthWiseConv_5, self).__init__()
...
class DepthWiseConv_7(nn.Module):
    def __init__(self, in_channel, out_channel):
        # 这一行千万不要忘记
        super(DepthWiseConv_7, self).__init__()
...
class MFA(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        self.cv = nn.Conv2d(c1, c2, 1, 1)
        # self.bn = nn.BatchNorm2d(c2, affine=False, track_running_stats=False)
        self.sigmoid = nn.Sigmoid()
        self.PConv = Partial_conv3(c2, 8, 'split_cat')
        self.DySnakeConv = DySnakeConv(c2, c2)
        # self.DCNv3_1 = DCNV3_YoLo(c2, c2, 3, 1, 1, g=8)
        self.C3_DySnake = C3_DySnake(c1, c1)
    ...
    def forward(self, x):
        ...
        # a = self.bn(a)
        a = self.sigmoid(a)
        out = self.DySnakeConv(self.PConv(x)) * a
        out = out + self.C3_DySnake(x)
        return out
...
class MS(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        self.cv = nn.Conv2d(c1, c1//2, 1, 1)
        self.cv2 = nn.Conv2d(3*c1//2, c2, 1, 1)
        # self.DCNv2 = DCNv2(c1//2, c1//2, 3, 1, 1)
        # self.DRConv = DRConv2d(c1//2, c1//2, 1, 1)
        self.PConv1 = Partial_conv3(c1//2, 2, 'split_cat')
        self.PConv2 = Partial_conv3(c1//2, 4, 'split_cat')
        self.PConv3 = Partial_conv3(c1//2, 8, 'split_cat')
        self.Conv1 = DWConv(c1//2, c1//2, 3, 1)
        self.Conv2 = DWConv(c1//2, c1//2, 5, 1)
        self.Conv3 = DWConv(c1//2, c1//2, 7, 1)

    def forward(self, x):
        a = self.Conv1(self.PConv1(self.cv(x)))
        b = self.Conv2(self.PConv2(self.cv(torch.cat((self.cv(x), self.PConv1(self.cv(x))), dim=1))))
        c = self.Conv3(self.PConv3(self.cv(torch.cat((self.cv(x), self.PConv2(self.cv(torch.cat(
            (self.cv(x), self.PConv1(self.cv(x))), dim=1)))), dim=1))))
...
class MS2(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2):
        super(MS2, self).__init__()
        self.cv = nn.Conv2d(c1, c1//2, 1, 1)
        self.cv2 = nn.Conv2d(3*c1//2, c2, 1, 1)
        # self.DCNv2 = DCNv2(c1//2, c1//2, 3, 1, 1)
        # self.DRConv = DRConv2d(c1//2, c1//2, 1, 1)
        self.DCNv3_1 = DCNV3_YoLo(c1//2, c1//2, 3, 1, 1, g=8, d=1)
        self.DCNv3_2 = DCNV3_YoLo(c1//2, c1//2, 3, 1, 3, g=8, d=3)
        self.DCNv3_3 = DCNV3_YoLo(c1//2, c1//2, 3, 1, 5, g=8, d=5)
        # self.PConv1 = Partial_conv3(c1//2, 2, 'split_cat')
        # self.PConv2 = Partial_conv3(c1//2, 4, 'split_cat')
        # self.PConv3 = Partial_conv3(c1//2, 8, 'split_cat')
        # self.Conv1 = DWConv(c1//2, c1//2, 3, 1)
        # self.Conv2 = DWConv(c1//2, c1//2, 5, 1)
        # self.Conv3 = DWConv(c1//2, c1//2, 7, 1)
        self.Conv1 = DepthWiseConv_3(c1 // 2, c1 // 2)
        self.Conv2 = DepthWiseConv_5(c1 // 2, c1 // 2)
        self.Conv3 = DepthWiseConv_7(c1 // 2, c1 // 2)

    def forward(self, x):
        a = self.Conv1(self.DCNv3_1(self.cv(x)))
        b = self.Conv2(self.DCNv3_2(self.cv(torch.cat((self.cv(x), self.DCNv3_1(self.cv(x))), dim=1))))
        c = self.Conv3(self.DCNv3_3(self.cv(torch.cat((self.cv(x), self.DCNv3_2(self.cv(torch.cat(
            (self.cv(x), self.DCNv3_1(self.cv(x))), dim=1)))), dim=1))))
        x = torch.cat((a, b, c), dim=1)
        x = self.cv2(x)
        return x
...
class MS_2(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        self.cv = nn.Conv2d(c1, c1//2, 1, 1)
        self.cv2 = nn.Conv2d(3*c1//2, c2, 1, 1)
        self.DCNv3 = DCNV3_YoLo(c1//2, c1//2, 3, 1, 1)
        # self.DRConv = DRConv2d(c1//2, c1//2, 1, 1)
        self.PConv1 = Partial_conv3(c1//2, 2, 'split_cat')
        self.PConv2 = Partial_conv3(c1//2, 4, 'split_cat')
        self.PConv3 = Partial_conv3(c1//2, 8, 'split_cat')
        self.Conv1 = DWConv(c1//2, c1//2, 3, 1, 1)
        self.Conv2 = DWConv(c1//2, c1//2, 5, 1, 2)
        self.Conv3 = DWConv(c1//2, c1//2, 7, 1, 3)

    def forward(self, x):
        a = self.Conv1(self.DCNv3(self.PConv1(self.cv(x))))
        b = self.Conv2(self.DCNv3(self.PConv2(self.cv(torch.cat((self.cv(x), self.DCNv3(self.PConv1(self.cv(x)))),
                                                                dim=1)))))
        c = self.Conv3(self.DCNv3(self.PConv3(self.cv(torch.cat((self.cv(x), self.DCNv3(self.PConv2(self.cv(torch.cat(
            (self.cv(x), self.DCNv3(self.PConv1(self.cv(x)))), dim=1))))), dim=1)))))
        x = torch.cat((a, b, c), dim=1)
        x = self.cv2(x)
        return x

#
# class MS1(nn.Module):         # MS2
#     # CSP Bottleneck with 3 convolutions
#     def __init__(self, c1, c2):
#         super(MS1, self).__init__()
#         self.cv = nn.Conv2d(c1, c1//2, 1, 1)
#         self.cv2 = nn.Conv2d(3*c1//2, c2, 1, 1)
#         self.DCNv3 = DCNV3_YoLo(c1//2, c1//2, 3, 1, 1)
#         # self.DRConv = DRConv2d(c1//2, c1//2, 1, 1)
...
#         self.PConv3 = Partial_conv3(c1//2, 8, 'split_cat')
#         self.Conv1 = DWConv(c1//2, c1//2, 3, 1, 1)
#         self.Conv2 = DWConv(c1//2, c1//2, 3, 1, 1)
#         self.Conv3 = DWConv(c1//2, c1//2, 3, 1, 1)
#
#     def forward(self, x):
#         a = self.Conv1(self.DCNv3(self.PConv1(self.cv(x))))
#         b = self.Conv2(self.DCNv3(self.PConv2(self.cv(torch.cat((self.cv(x), self.DCNv3(self.PConv1(self.cv(x)))),
...
#         self.PConv2 = Partial_conv3(c1//2, 4, 'split_cat')
#         self.PConv3 = Partial_conv3(c1//2, 8, 'split_cat')
#         self.Conv1 = DWConv(c1//2, c1//2, 5, 1, 2)
#         self.Conv2 = DWConv(c1//2, c1//2, 5, 1, 2)
#         self.Conv3 = DWConv(c1//2, c1//2, 5, 1, 2)
#
#     def forward(self, x):
#         a = self.Conv1(self.DCNv3(self.PConv1(self.cv(x))))
#         b = self.Conv2(self.DCNv3(self.PConv2(self.cv(torch.cat((self.cv(x), self.DCNv3(self.PConv1(self.cv(x)))),
...
#         return x
#
# class MS3(nn.Module):      # MS2
#     # CSP Bottleneck with 3 convolutions
#     def __init__(self, c1, c2):
#         super(MS3, self).__init__()
#         self.cv = nn.Conv2d(c1, c1//2, 1, 1)
#         self.cv2 = nn.Conv2d(3*c1//2, c2, 1, 1)
#         self.DCNv3 = DCNV3_YoLo(c1//2, c1//2, 3, 1, 1)
#         # self.DRConv = DRConv2d(c1//2, c1//2, 1, 1)
...
#         self.PConv2 = Partial_conv3(c1//2, 4, 'split_cat')
#         self.PConv3 = Partial_conv3(c1//2, 8, 'split_cat')
#         self.Conv1 = DWConv(c1//2, c1//2, 7, 1, 3)
#         self.Conv2 = DWConv(c1//2, c1//2, 7, 1, 3)
#         self.Conv3 = DWConv(c1//2, c1//2, 7, 1, 3)
#
#     def forward(self, x):
#         a = self.Conv1(self.DCNv3(self.PConv1(self.cv(x))))
...
class CSPConv(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(4 * c_, c2, 1)  # act=FReLU(c2)
        self.cv4 = Conv(c_, c_, 3, 1, g=g)    # 新加
